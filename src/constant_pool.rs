...
pub(crate) enum ConstantPoolRef {
    Unresolved(u16),
    Resolved(Arc<ConstantPoolEntry>),
}
...
impl ConstantPoolRef {
    ...
    fn get(&self) -> &Arc<ConstantPoolEntry> {
        match self {
...
impl RwLockDeref for RwLock<ConstantPoolRef> {
    fn resolve(
        &self,
        cp_index: usize,
        pool: &[Arc<ConstantPoolEntry>],
    ) -> Result<(), ParseError> {
        self.write().unwrap().resolve(cp_index, pool)
    }

    fn ensure_type(&self, allowed: ConstantPoolEntryTypes) -> Result<(), ParseError> {
        self.read().unwrap().get().ensure_type(allowed)
    }
}
...
pub(crate) enum ConstantPoolEntry {
    ...
    Long(i64),
    Double(f64),
    ClassInfo(RwLock<ConstantPoolRef>),
    String(RwLock<ConstantPoolRef>),
    FieldRef(RwLock<ConstantPoolRef>, RwLock<ConstantPoolRef>),
    MethodRef(RwLock<ConstantPoolRef>, RwLock<ConstantPoolRef>),
    InterfaceMethodRef(RwLock<ConstantPoolRef>, RwLock<ConstantPoolRef>),
    NameAndType(RwLock<ConstantPoolRef>, RwLock<ConstantPoolRef>),
    MethodHandle(ReferenceKind, RwLock<ConstantPoolRef>),
    MethodType(RwLock<ConstantPoolRef>),
    Dynamic(BootstrapMethodRef, RwLock<ConstantPoolRef>),
    InvokeDynamic(BootstrapMethodRef, RwLock<ConstantPoolRef>),
    ModuleInfo(RwLock<ConstantPoolRef>),
    PackageInfo(RwLock<ConstantPoolRef>),
    Unused,
}
...
impl ConstantPoolEntry {
    ...
    fn validate(&self, major_version: u16) -> Result<(), ParseError> {
        match self {
            ConstantPoolEntry::ClassInfo(x) => {
                ...
                x.read().unwrap().get().validate_classinfo_name()
            }
            ...
            ConstantPoolEntry::FieldRef(x, y) => {
                ...
                y.read().unwrap().get().validate_field_descriptor()
            }
            ConstantPoolEntry::MethodRef(x, y) => {
                x.ensure_type(ConstantPoolEntryTypes::CLASS_INFO)?;
                y.ensure_type(ConstantPoolEntryTypes::NAME_AND_TYPE)?;
                y.read().unwrap().get().validate_method_descriptor()
            }
            ConstantPoolEntry::InterfaceMethodRef(x, y) => {
                x.ensure_type(ConstantPoolEntryTypes::CLASS_INFO)?;
                y.ensure_type(ConstantPoolEntryTypes::NAME_AND_TYPE)?;
            ...
            ConstantPoolEntry::NameAndType(x, y) => {
                x.ensure_type(ConstantPoolEntryTypes::UTF8)?;
                x.read().unwrap().get().validate_unqualified_name()?;
            ...
            ConstantPoolEntry::MethodType(x) => {
                x.ensure_type(ConstantPoolEntryTypes::UTF8)?;
                x.read().unwrap().get().validate_method_descriptor()
            }
            ...
            ConstantPoolEntry::ModuleInfo(x) => {
                x.ensure_type(ConstantPoolEntryTypes::UTF8)?;
                x.read().unwrap().get().validate_module_name()
            }
            ConstantPoolEntry::PackageInfo(x) => {
                x.ensure_type(ConstantPoolEntryTypes::UTF8)?;
                x.read().unwrap().get().validate_binary_name()
            }
    ...
    fn validate_classinfo_name(&self) -> Result<(), ParseError> {
        ...
        if (x.chars().next().map(|chr|chr == 'L').unwrap_or(false) && x.chars().last().map(|chr|chr == ';').unwrap_or(false)) || is_binary_name(x) || is_array_descriptor(x) {
            Ok(())
        } else {
    ...
    fn validate_field_descriptor(&self) -> Result<(), ParseError> {
        match self {
            ConstantPoolEntry::NameAndType(_, y) => {
                if is_field_descriptor(y.read().unwrap().get().str()?) {
                    Ok(())
                } else {
    ...
    fn validate_method_descriptor(&self) -> Result<(), ParseError> {
        match self {
            ConstantPoolEntry::NameAndType(_, y) => y.read().unwrap().get().validate_method_descriptor(),
            _ => {
    ...
    fn string_literal(&self) -> LiteralConstant {
        match self {
            ...
            ConstantPoolEntry::Utf8Bytes(x) => LiteralConstant::StringBytes(x.clone()),
            _ => panic!("Attempting to get utf-8 data from non-utf8 constant pool entry!"),
        }
    }

    fn classinfo(&self) -> Arc<str> {
        match self {
            ConstantPoolEntry::ClassInfo(x) => x.read().unwrap().get().utf8(),
    ...
    fn name_and_type(&self) -> NameAndType {
        match self {
            ConstantPoolEntry::NameAndType(x, y) => NameAndType {
                name: x.read().unwrap().get().utf8(),
                descriptor: y.read().unwrap().get().utf8(),
            },
            _ => panic!(
...
fn read_unresolved_cp_ref(
    bytes: & [u8],
    ix: &mut usize,
) -> Result<RwLock<ConstantPoolRef>, ParseError> {
    Ok(RwLock::new(ConstantPoolRef::Unresolved(read_u2(
        bytes, ix,
    )?)))
}

fn read_constant_utf8(
    bytes: & [u8],
    ix: &mut usize,
    ...
    *ix += length;
    ...
    match cesu8::from_java_cesu8(modified_utf8_data) {
        Ok(rust_str) => Ok(ConstantPoolEntry::Utf8(Arc::from(rust_str))),
        _ => Ok(ConstantPoolEntry::Utf8Bytes(Arc::from(modified_utf8_data))),
    }
}

fn read_constant_integer(
    bytes: & [u8],
    ix: &mut usize,
) -> Result<ConstantPoolEntry, ParseError> {
...
fn read_constant_interfacemethodref(
    bytes: & [u8],
    ix: &mut usize,
) -> Result<ConstantPoolEntry, ParseError> {
    let class_ref = read_unresolved_cp_ref(bytes, ix)?;
...
fn read_constant_invokedynamic(
    bytes: & [u8],
    ix: &mut usize,
) -> Result<ConstantPoolEntry, ParseError> {
    let bootstrap_method_ref = read_u2(bytes, ix)?;
...
fn resolve_constant_pool(constant_pool: &[Arc<ConstantPoolEntry>]) -> Result<(), ParseError> {
...
fn validate_constant_pool(
    constant_pool: &[Arc<ConstantPoolEntry>],
    major_version: u16,
) -> Result<(), ParseError> {
    for (i, cp_entry) in constant_pool.iter().enumerate() {
        cp_entry
...
pub(crate) fn read_constant_pool(
    ...
    while cp_ix < count {
        ...
        constant_pool.push(Arc::new(match constant_type {
        ...
        if constant_type == 5 || constant_type == 6 {
            ...
            cp_ix += 1;
            constant_pool.push(Arc::new(ConstantPoolEntry::Unused));
        }
    }
...
fn read_cp_ref_any(
    ...
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Arc<ConstantPoolEntry>, ParseError> {
...
pub(crate) fn read_cp_classinfo(
    bytes: &[u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Arc<str>, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ConstantPoolEntry::ClassInfo(x) => Ok(x.read().unwrap().get().utf8()),
        _ => fail!("Unexpected constant pool reference type"),
    }
}

pub(crate) fn read_cp_classinfo_opt(
    bytes: &[u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Option<Arc<str>>, ParseError> {
    ...
    match cp_ref.deref() {
        ConstantPoolEntry::Zero => Ok(None),
        ConstantPoolEntry::ClassInfo(x) => Ok(Some(x.read().unwrap().get().utf8())),
        _ => fail!("Unexpected constant pool reference type"),
    }
}

pub(crate) fn read_cp_moduleinfo(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Arc<str>, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ConstantPoolEntry::ModuleInfo(x) => Ok(x.read().unwrap().get().utf8()),
        _ => fail!("Unexpected constant pool reference type"),
    }
}

pub(crate) fn read_cp_packageinfo(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Arc<str>, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ConstantPoolEntry::PackageInfo(x) => Ok(x.read().unwrap().get().utf8()),
        _ => fail!("Unexpected constant pool reference type"),
    }
}
...
pub(crate) fn read_cp_nameandtype_opt(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Option<NameAndType>, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ConstantPoolEntry::Zero => Ok(None),
...
pub(crate) fn read_cp_literalconstant(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<LiteralConstant, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ...
        ConstantPoolEntry::String(v) => Ok(v.read().unwrap().get().string_literal()),
        _ => fail!("Unexpected constant pool reference type"),
    }
}

pub(crate) fn read_cp_integer(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
...
pub(crate) fn read_cp_memberref(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
    ...
    match cp_ref.deref() {
        ...
        | ConstantPoolEntry::InterfaceMethodRef(c, m) => Ok(MemberRef {
            class_name: c.read().unwrap().get().classinfo(),
            name_and_type: m.read().unwrap().get().name_and_type(),
        }),
        _ => fail!("Unexpected constant pool reference type"),
    }
}
...
pub(crate) fn read_cp_invokedynamic(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<InvokeDynamic, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ConstantPoolEntry::InvokeDynamic(x, y) => Ok(InvokeDynamic {
            attr_index: *x,
            name_and_type: y.read().unwrap().get().name_and_type(),
        }),
        _ => fail!("Unexpected constant pool reference type"),
    }
}
...
pub(crate) fn get_cp_loadable(
    cp_index: usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<Loadable, ParseError> {
    if cp_index >= pool.len() {
        fail!(
            "Out-of-bounds index {} in constant pool reference",
            cp_index
    ...
    match pool[cp_index].deref() {
        ...
        ConstantPoolEntry::String(v) => {
            Ok(Loadable::LiteralConstant(v.read().unwrap().get().string_literal()))
        }
        ConstantPoolEntry::ClassInfo(x) => Ok(Loadable::ClassInfo(x.read().unwrap().get().utf8())),
        ConstantPoolEntry::MethodHandle(x, y) => {
            Ok(Loadable::MethodHandle(make_method_handle(x, y)?))
        }
        ConstantPoolEntry::MethodType(x) => Ok(Loadable::MethodType(x.read().unwrap().get().utf8())),
...
fn make_method_handle(
    x: &ReferenceKind,
    y: &RwLock<ConstantPoolRef>,
) -> Result<MethodHandle, ParseError> {
    let (class_name, member_kind, member_ref) = match y.read().unwrap().get().deref() {
        ConstantPoolEntry::FieldRef(c, m) => (
            c.read().unwrap().get().classinfo(),
            MemberKind::Field,
            m.read().unwrap().get().name_and_type(),
        ),
        ConstantPoolEntry::MethodRef(c, m) => (
            c.read().unwrap().get().classinfo(),
            MemberKind::Method,
...
pub(crate) fn read_cp_methodhandle(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<MethodHandle, ParseError> {
...
pub(crate) fn read_cp_bootstrap_argument(
    bytes: & [u8],
    ix: &mut usize,
    pool: &[Arc<ConstantPoolEntry>],
) -> Result<BootstrapArgument, ParseError> {
    let cp_ref = read_cp_ref_any(bytes, ix, pool)?;
    match cp_ref.deref() {
        ...
        ConstantPoolEntry::String(v) => Ok(BootstrapArgument::LiteralConstant(
            v.read().unwrap().get().string_literal(),
        )),
        ConstantPoolEntry::ClassInfo(x) => {
            Ok(BootstrapArgument::ClassInfo(x.read().unwrap().get().utf8()))
        }
        ConstantPoolEntry::MethodHandle(x, y) => {
            Ok(BootstrapArgument::MethodHandle(make_method_handle(x, y)?))
        }
        ConstantPoolEntry::MethodType(x) => {
            Ok(BootstrapArgument::MethodType(x.read().unwrap().get().utf8()))
        }
        _ => fail!("Unexpected constant pool reference type"),
    }
}
...
impl ConstantPoolIter {
    pub(crate) fn new(constant_pool: Arc<[Arc<ConstantPoolEntry>]>) -> Self {
        ConstantPoolIter {
            constant_pool,
            index: 0,
        }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.index + 1 < self.constant_pool.len() {
            ...
            let item = match self.constant_pool[self.index].deref() {
                ...
                ConstantPoolEntry::ClassInfo(x) => {
                    ConstantPoolItem::ClassInfo(x.read().unwrap().get().utf8())
                }
                ConstantPoolEntry::String(x) => {
                    ConstantPoolItem::LiteralConstant(x.read().unwrap().get().string_literal())
                }
                ...
                ConstantPoolEntry::MethodType(x) => {
                    ConstantPoolItem::MethodType(x.read().unwrap().get().utf8())
                }
                ...
                ConstantPoolEntry::ModuleInfo(x) => {
                    ConstantPoolItem::ModuleInfo(x.read().unwrap().get().utf8())
                }
                ConstantPoolEntry::PackageInfo(x) => {
                    ConstantPoolItem::PackageInfo(x.read().unwrap().get().utf8())
                }
